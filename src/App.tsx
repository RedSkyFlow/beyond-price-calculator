...
export default function App() {
  ...
  }, [vertical, measureValue, avgApsPerVenue, apCount, selectedPackage, addOns, term, hotelRooms]);
  ...
  const handleExportProposal = async () => {
    ...
    try {
      ...
      // TCV Highlight (Starting at y=76 with height 30 instead of 40)
      ...
      doc.setFontSize(8);
      doc.setFont('helvetica', 'bold');
      doc.text('TOTAL CONTRACT VALUE (TCV)', 25, 84);
      
      doc.setFontSize(20);
      doc.text(formatCurrency(calculation.tcv), 25, 96);
      
      doc.setTextColor(100, 100, 100);
      doc.setFontSize(11);
      doc.text(formatZar(calculation.tcv), pageWidth - 25, 96, { align: 'right' });
      ...
      // Cost Breakdown Table (Starting at y=112 instead of 140)
      autoTable(doc, {
        startY: 112,
        head: [['Item', 'Type', 'Annual Price (GBP)', 'Annual Price (ZAR)']],
        body: [
          [`${selectedPackage} License`, 'License', formatCurrency(calculation.baseAnnual), formatZar(calculation.baseAnnual)],
          ...(calculation.success > 0 ? [['Success (Managed Service)', 'Service', formatCurrency(calculation.success), formatZar(calculation.success)]] : []),
          ...(calculation.shieldCost > 0 ? [['Shield Module', 'Add-on', formatCurrency(calculation.shieldCost), formatZar(calculation.shieldCost)]] : []),
          ...(calculation.surveysCost > 0 ? [['Surveys Module', 'Add-on', formatCurrency(calculation.surveysCost), formatZar(calculation.surveysCost)]] : []),
          [{ content: 'Total Annual Recurring', colSpan: 2, styles: { fontStyle: 'bold' } }, 
           { content: formatCurrency(calculation.totalAnnual), styles: { fontStyle: 'bold', textColor: [99, 33, 255] } },
           { content: formatZar(calculation.totalAnnual), styles: { fontStyle: 'bold', textColor: [99, 33, 255] } }]
        ],
        theme: 'striped',
        headStyles: { fillColor: [27, 32, 60], textColor: [255, 255, 255] },
        styles: { fontSize: 8.5, cellPadding: 4 }, // Reduced font size and cell padding slightly to save vertical space
        columnStyles: {
          2: { halign: 'right' },
          3: { halign: 'right' }
        }
      ...
      doc.setFillColor(248, 249, 252);
      doc.roundedRect(15, enablementY, pageWidth - 30, 14, 2, 2, 'F'); // Reduced height from 20 to 14
      
      doc.setTextColor(27, 32, 60);
      doc.setFontSize(9.5);
      doc.setFont('helvetica', 'bold');
      doc.text('One-off Enablement Fee:', 25, enablementY + 9.5);
      doc.text(`${formatCurrency(calculation.enablement)} / ${formatZar(calculation.enablement)}`, pageWidth - 25, enablementY + 9.5, { align: 'right' });
      ...
      autoTable(doc, {
        ...
        head: [['Analysis Level', 'Monthly Cost', 'Daily Cost']],
        body: [
          [`Per ${calculation.measure}`, `${formatCurrency(calculation.perVenueMonthly)} / ${formatZar(calculation.perVenueMonthly)}`, `${formatCurrency(calculation.perVenueDaily)} / ${formatZar(calculation.perVenueDaily)}`],
          [`Total Deal (${vertical === 'Hotels' ? `${measureValue} Hotels / ${hotelRooms} Rooms` : `${calculation.venueCount} ${calculation.measure}s`})`, `${formatCurrency(calculation.totalMonthly)} / ${formatZar(calculation.totalMonthly)}`, `${formatCurrency(calculation.totalDaily)} / ${formatZar(calculation.totalDaily)}`]
        ],
        theme: 'grid',
        headStyles: { fillColor: [99, 33, 255], textColor: [255, 255, 255] },
        styles: { fontSize: 8.5, cellPadding: 4 } // Reduced cell padding slightly to save vertical space
      });
                ...
                <Card className="border-none shadow-xl shadow-blue-900/5 bg-white overflow-hidden">
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="bg-gradient-to-r from-[#6321FF]/5 to-[#6321FF]/10 p-6 rounded-2xl border border-[#6321FF]/10 mb-6">
                      ...
                      <p className="text-xs text-[#6321FF]/60 mt-2 font-medium">
                    ...
                    <Table>
                      ...
                      <TableBody>
                        <TableRow>
                          ...
                          <TableCell className="text-right">{formatCurrency(calculation?.baseAnnual || 0)}</TableCell>
                          <TableCell className="text-right text-slate-500">{formatZar(calculation?.baseAnnual || 0)}</TableCell>
                        </TableRow>
                        {(calculation?.success || 0) > 0 && (
                          <TableRow>
                            ...
                            <TableCell className="text-right">{formatCurrency(calculation?.success || 0)}</TableCell>
                            <TableCell className="text-right text-slate-500">{formatZar(calculation?.success || 0)}</TableCell>
                          </TableRow>
                        )}
                        {(calculation?.shieldCost || 0) > 0 && (
                          <TableRow>
                            ...
                            <TableCell className="text-right">{formatCurrency(calculation?.shieldCost || 0)}</TableCell>
                            <TableCell className="text-right text-slate-500">{formatZar(calculation?.shieldCost || 0)}</TableCell>
                          </TableRow>
                        )}
                        {(calculation?.surveysCost || 0) > 0 && (
                          <TableRow>
                            ...
                            <TableCell className="text-right">{formatCurrency(calculation?.surveysCost || 0)}</TableCell>
                            <TableCell className="text-right text-slate-500">{formatZar(calculation?.surveysCost || 0)}</TableCell>
                          </TableRow>
                        )}
                        <TableRow className="border-t-2 border-[#6321FF]/20">
                          <TableCell className="font-bold text-[#6321FF]">Total Annual Recurring</TableCell>
                          <TableCell className="text-right font-bold text-[#6321FF]">{formatCurrency(calculation?.totalAnnual || 0)}</TableCell>
                          <TableCell className="text-right font-bold text-[#6321FF]/70">{formatZar(calculation?.totalAnnual || 0)}</TableCell>
                        </TableRow>
                      </TableBody>
                    </Table>
                    ...
                    <div className="mt-4 p-4 bg-slate-50 rounded-xl border border-slate-100 flex justify-between items-center">
                      ...
                      <div className="text-right">
                        <p className="text-lg font-black">{formatCurrency(calculation?.enablement || 0)}</p>
                        <p className="text-[10px] font-bold text-slate-400">{formatZar(calculation?.enablement || 0)}</p>
                      </div>
                    </div>
                  </CardContent>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="p-3 bg-slate-50 rounded-xl">
                            ...
                            <p className="text-lg font-black text-slate-900">{formatCurrency(calculation?.perVenueMonthly || 0)}</p>
                            <p className="text-[10px] font-bold text-slate-400">{formatZar(calculation?.perVenueMonthly || 0)}</p>
                          </div>
                          <div className="p-3 bg-slate-50 rounded-xl">
                            <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">Daily</p>
                            <p className="text-lg font-black text-slate-900">{formatCurrency(calculation?.perVenueDaily || 0)}</p>
                            <p className="text-[10px] font-bold text-slate-400">{formatZar(calculation?.perVenueDaily || 0)}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="p-3 bg-[#6321FF]/5 rounded-xl border border-[#6321FF]/10">
                            ...
                            <p className="text-lg font-black text-[#6321FF]">{formatCurrency(calculation?.totalMonthly || 0)}</p>
                            <p className="text-[10px] font-bold text-[#6321FF]/60">{formatZar(calculation?.totalMonthly || 0)}</p>
                          </div>
                          <div className="p-3 bg-[#6321FF]/5 rounded-xl border border-[#6321FF]/10">
                            <p className="text-[10px] text-[#6321FF] uppercase font-bold tracking-wider mb-1">Daily</p>
                            <p className="text-lg font-black text-[#6321FF]">{formatCurrency(calculation?.totalDaily || 0)}</p>
                            <p className="text-[10px] font-bold text-[#6321FF]/60">{formatZar(calculation?.totalDaily || 0)}</p>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                {calculation && (vertical in MULTI_VENUE_ICP || calculation.isNonICP) && (
                  <div className="p-4 bg-amber-50 border border-amber-100 rounded-xl flex gap-3">
                    ...
                    <p className="text-xs text-amber-800 leading-relaxed">
                      ...
                      Annual price = {formatCurrency((calculation.baseAnnual || 0) / (calculation.isNonICP ? (vertical in MULTI_VENUE_ICP ? (measureValue * avgApsPerVenue || 1) : (apCount || 1)) : (measureValue || 1)))} ({formatZar((calculation.baseAnnual || 0) / (calculation.isNonICP ? (vertical in MULTI_VENUE_ICP ? (measureValue * avgApsPerVenue || 1) : (apCount || 1)) : (measureValue || 1)))}) per {calculation.measure} x {calculation.isNonICP ? (vertical in MULTI_VENUE_ICP ? (measureValue * avgApsPerVenue) : apCount) : measureValue}.
                    </p>
                  </div>
                )}
              </motion.div>
